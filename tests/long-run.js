...
for(const mode of modes){
  ...
  while(ev('sim.generation')<=3&&guard<50000){ev('step();');ticks++;guard++;const g=ev('sim.generation');if(g!==lastGen){perGeneration.push({completed:lastGen,total:numericSnapshot(mode),rounds:ev(`sim.lifetimeRounds.${mode}`)});lastGen=g;}}
  const ms=Number(process.hrtime.bigint()-start)/1e6;
  assert.ok(ev('sim.generation')>=4,`${mode}: three-generation / twelve-trial playtest did not complete`);
  assert.strictEqual(ev(`sim.lifetimeRounds.${mode}`),12,`${mode}: three generations should contain twelve trials`);
  ...
  if(mode==='target')assert.ok(Object.values(totals).reduce((a,b)=>a+b,0)>0,'Target Practice produced zero hits across twelve trials');
  if(mode==='invaders')assert.ok(Object.values(totals).reduce((a,b)=>a+b,0)>0,'Invaders produced zero kills across twelve trials');
  if(mode==='royale')assert.ok(Object.values(totals).reduce((a,b)=>a+b.kills,0)>0,'Battle Royale produced zero kills across twelve trials');
