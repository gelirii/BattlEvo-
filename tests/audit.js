...
assert.strictEqual(ev('INPUTS'),599,'RC5 tactical input count changed unexpectedly');
...
// Tactical sight is 360°; only bunker occlusion removes a dynamic object.
reset('target');ev(`(()=>{const a=sim.agents.find(x=>x.species.id==='red');a.x=400;a.y=300;a.facing=0;sim.bunkers=[];})()`);
assert.strictEqual(ev("lineOfSight(sim.agents.find(x=>x.species.id==='red'),{x:260,y:300})"),true,'object behind facing direction should still be visible');
ev(`sim.bunkers=[newBunker(320,270,45,60,'LOS')];`);assert.strictEqual(ev("lineOfSight(sim.agents.find(x=>x.species.id==='red'),{x:260,y:300})"),false,'bunker should occlude dynamic object');
assert.strictEqual(ev('typeof inFront'),'undefined');assert.strictEqual(ev('typeof rememberVisibleBunkers'),'undefined');assert.strictEqual(ev('typeof sensorSector'),'undefined');assert.strictEqual(ev('typeof relativeVelocity'),'undefined');
...
const INPUTS_N=ev('INPUTS'),genome=n=>INPUTS_N*n+n+n*18+18,rate=n=>.06*Math.sqrt(genome(4)/genome(n));assert.ok(rate(20)<rate(4)&&rate(64)<rate(20));
console.log('BattlEvo v1 gameplay audit passed.');console.log(JSON.stringify({inputs:INPUTS_N,speedRatio:+ratio.toFixed(3),stationary100pxReactionSeconds:+(reaction100/60).toFixed(3),lateralDodgeAt100px:+lateral100.toFixed(1),fleeing100pxCatchSeconds:+(fleeCatch100/60).toFixed(3),battlefieldStartProgress:starts.map(x=>+x.toFixed(4)),coverLayouts:coverSignatures,invaderShotsPer10sPerTeam:perTeam},null,2));
