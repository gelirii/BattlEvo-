...
assert.strictEqual(ev('INPUTS'),77);assert.strictEqual(ev('OUTPUTS'),18);assert.strictEqual(ev('MIN_HIDDEN'),1);assert.strictEqual(ev('MAX_HIDDEN'),64);assert.strictEqual(ev('POP_SIZE'),16);assert.strictEqual(ev('TRIALS_PER_GENERATION'),2);assert.strictEqual(ev('FIELD.size'),ev('H'));assert.ok(ev('PROJECTILE_SPEED>AGENT_SPEED'));

for(const mode of ['target','battlefield','invaders','royale']){
  init(mode);assert.strictEqual(ev('sim.agents.length'),48,`${mode}: expected 48 creatures`);assert.strictEqual(ev('buildInputs(sim.agents[0]).length'),77,`${mode}: sensory vector mismatch`);assert.ok(ev('sim.agents.every(a=>inArenaPoint(a))'),`${mode}: spawn outside field`);
...
// Same sixteen physical Target Practice starts for all colours; genotype assignment may differ.
...
// Engine-level neuron validation.
...
// One genetic generation now contains exactly two complete evaluation trials.
elements['brain-red'].value=elements['brain-green'].value=elements['brain-blue'].value='2';
for(const mode of ['target','battlefield','invaders','royale']){
  init(mode);let guard=0;while(ev('sim.generation')===1&&guard<6000){ev('step();');guard++;}
  assert.strictEqual(ev('sim.generation'),2,`${mode}: generation did not finish after two trials`);assert.strictEqual(ev('sim.trial'),1,`${mode}: next generation did not reset to trial 1`);assert.ok(ev('sim.lastSummary.length')>5,`${mode}: missing summary`);assert.strictEqual(ev(`sim.lifetimeRounds.${mode}`),2,`${mode}: expected two counted rounds per generation`);
}
...
console.log('BattlEvo RC2 production smoke test passed.');