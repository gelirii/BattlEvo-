...
assert.strictEqual(ev('INPUTS'),77);assert.strictEqual(ev('OUTPUTS'),18);assert.strictEqual(ev('MIN_HIDDEN'),1);assert.strictEqual(ev('MAX_HIDDEN'),64);assert.strictEqual(ev('FIELD.size'),ev('H'));assert.ok(ev('PROJECTILE_SPEED>AGENT_SPEED'));

for(const mode of ['target','battlefield','invaders','royale']){
  init(mode);assert.strictEqual(ev('sim.agents.length'),36,`${mode}: expected 36 creatures`);assert.strictEqual(ev('buildInputs(sim.agents[0]).length'),77,`${mode}: sensory vector mismatch`);assert.ok(ev('sim.agents.every(a=>inArenaPoint(a))'),`${mode}: spawn outside field`);
  for(let i=0;i<180;i++)ev('step();');assert.ok(ev('sim&&sim.generation>=1'),`${mode}: runtime stopped`);
}

// The actual browser Target Practice implementation must be the one exercised here.
...
// Same twelve physical Target Practice starts for all colours; genotype assignment may differ.
...
// Engine-level neuron validation: no silent 5,000,000N brain and zero clamps to one.
...
// Complete a real production generation in every scenario.
elements['brain-red'].value=elements['brain-green'].value=elements['brain-blue'].value='2';
for(const mode of ['target','battlefield','invaders','royale']){init(mode);let guard=0;while(ev('sim.generation')===1&&guard<2800){ev('step();');guard++;}assert.strictEqual(ev('sim.generation'),2,`${mode}: generation did not finish`);assert.ok(ev('sim.lastSummary.length')>5,`${mode}: missing summary`);assert.strictEqual(ev(`sim.lifetimeRounds.${mode}`),1,`${mode}: round counter did not increment`);}
...
console.log('BattlEvo production smoke test passed.');
