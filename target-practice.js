...
setupTarget=function(){
  ...
  // between colours. Every creature is independently placed without a species owning
  // a hard-coded edge, side or band of the arena.
  const spawnPoints=[];
...
// Motion stays readable rather than becoming Brownian: targets travel straight, bounce,
// and occasionally turn by 45 degrees. Initial heading, timing and handedness vary.
stepTargets=function(){
  for(const t of sim.targets){
    if(sim.tick>=t.nextTurn){
      const nd=vecToDir(t.dx,t.dy),d=DIRS[(nd+t.turnBias+8)%8];
      t.dx=d.x;t.dy=d.y;t.nextTurn+=t.turnEvery;
