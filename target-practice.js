...
function targetRectOverlaps(a,b,pad=0){return rectOverlap(a,b,pad);}
function targetPointClear(p,r,points=[],pointGap=0){
...
function randomTargetPoint(r,points=[],gap=0){
  for(let tries=0;tries<600;tries++){
    const p={x:rand(FIELD.right-r-20,FIELD.left+r+20),y:rand(FIELD.bottom-r-20,FIELD.top+r+20),r};
    if(targetPointClear(p,r,points,gap))return p;
  }
  return null;
}
function buildTargetLayout(){
  for(let attempt=0;attempt<50;attempt++){
    const targetDirs=shuffledIndices(8).slice(0,6),targets=[];let failed=false;
    for(let i=0;i<6;i++){
      const p=randomTargetPoint(9,targets,34);if(!p){failed=true;break;}
    ...
    for(let i=0;i<POP_SIZE;i++){
      const p=randomTargetPoint(AGENT_R,occupied,18);if(!p){failed=true;break;}
      starts.push({x:p.x,y:p.y,r:AGENT_R,facing:randi(8)});occupied.push({x:p.x,y:p.y,r:AGENT_R});
    }
    if(failed)continue;
    try{
      const bunkers=placeRandomBunkers(3,{prefix:'T',minSize:46,maxSize:82,edgeMargin:28,gap:34,clearance:16,avoid:[...targets,...starts]});
...
function setupTarget(){
  ...
  // All species receive the exact same sixteen physical starts. Genotypes are shuffled
