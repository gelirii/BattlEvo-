...
function inFront(agent,obj){
  const dx=obj.x-agent.x,dy=obj.y-agent.y,f=DIRS[agent.facing];
  return dot(dx,dy,f.x,f.y)>=0;
}

// Liang-Barsky segment/rectangle test. Centre-line occlusion is intentionally simple:
// if the line from an agent to an object's centre crosses a bunker, that object is hidden.
...
function buildInputs(a){
  ...
  // Seven sectors cover the full 180° forward field. Every mode speaks the same
  // combat vocabulary so skills can transfer: hostile proximity + velocity,
  // friendly proximity, incoming projectile proximity + velocity, and visible cover.
  ...
  const sectorOf=(o,ignoreBunkerId=null)=>{
    ...
    const dx=o.x-a.x,dy=o.y-a.y,d=Math.hypot(dx,dy);if(d<1)return null;
  ...
  const addBunker=(b)=>{
    const o={x:b.x+b.w/2,y:b.y+b.h/2},q=sectorOf(o,b.id);
  ...
  if(sim.mode==='invaders')for(const n of sim.invaders){
    if(n.aliveFor?.[a.species.id])addEnemy(n,n.vx||0,n.vy||0);
  }
  for(const s of sectors)for(const v of s)input[k++]=v;

  // Full 360° remembered terrain ring. A bunker only enters memory after it has
  // actually appeared in the forward field, then remains available after turning away.
  ...
  input[k++]=clamp((a.x-FIELD.cx)/(FIELD.size/2),-1,1);
  input[k++]=clamp((a.y-FIELD.cy)/(FIELD.size/2),-1,1);
  input[k++]=Math.sin(sim.tick*.025);input[k++]=Math.cos(sim.tick*.025);
...
function tryMove(a){
  ...
  const nx=clamp(a.x+d.x*AGENT_SPEED,FIELD.left+AGENT_R,FIELD.right-AGENT_R);
  const ny=clamp(a.y+d.y*AGENT_SPEED,FIELD.top+AGENT_R,FIELD.bottom-AGENT_R);
...
function stepAgents(){
  for(const a of sim.agents){
    ...
    // Battlefield Run is pure movement/threat training; its fire output is ignored.
...
function finishInvaderTeam(team){
  if(sim.invaderCleared[team])return;
  if(sim.invaders.some(n=>n.aliveFor?.[team]))return;
  sim.invaderCleared[team]=true;
  for(const a of sim.agents){
    if(a.species.id===team&&a.alive){a.fitness+=300;a.finished=true;a.alive=false;}
  }
}
