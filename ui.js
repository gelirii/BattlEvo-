...
function updateStats(){
  ...
  for(const s of SPECIES){
    ...
    const allTime=Number.isFinite(sim.bestEver[s.id])?sim.bestEver[s.id].toFixed(1):'—';
    ...
    document.getElementById('stat-'+s.id).textContent=`Alive ${alive}/${POP_SIZE} · best ${bestNow.toFixed(1)} · all-time ${allTime}${extra}`;
  }
}
function setRunningUI(on){
  ...
  // Training-ground buttons deliberately stay live. Changing ground keeps the
  // evolved populations and only resets the current arena/episode.
